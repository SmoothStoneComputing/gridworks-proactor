...
class LinkManager:
    ...
    def generate_event(self, event: EventT) -> Result[bool, Exception]:
        num_pending_dbg = len(self._in_flight_events)
        ...
        if (
            ...
            and self._states[self._mqtt_clients.upstream_client].active_for_new_events()
        ):
            path_dbg |= 0x00000008
        ...
        self._logger.path(
            "--generate_event %s  path:0x%08X  %d - %d  (%d)",
            event.TypeName,
            path_dbg,
            num_pending_dbg,
    ...
    def process_ack_timeout(
        ...
        match state_result := self._states.process_ack_timeout(wait_info.link_name):
            case Ok():
                ...
                elif transition.optimistic_send_timed_out():
                    path_dbg |= 0x00000004
                    # We intentionally leave ack timers running. There be non-event
                    # messages requiring ack, and we want any handlers for their
                    # timeouts to fire.
                    self.flush_in_flight_events()
                    self._logger.comm_event(str(transition))
